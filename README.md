...
3. Ketika memanggil makemigrations, Django akan secara otomatis membuat file migration yang berisi model yang sudah kita buat. Sementara itu, command migrate akan memberitahu Django file migration mana yang harus digunakan. Secara default, Django akan menggunakan file migration terakhir yang dibuat. File ini juga berisi dependensi terhadap file-file sebelumnya. Jadi, Django tidak hanya menggunakan model yang ada pada file yang kita pilih saat menjalankan command migrate, tapi juga menggunakan model yang ada di file-file migrations sebelumnya. Sebagai contoh, ketika saya memanggil makemigrations setelah membuat model data skill, Django langsung membuat file migration yang berisikan model skill yang sudah saya buat. Setelah itu, saya menjalankan command migrate untuk memberitahu Django untuk menggunakan file migration tersebut.
